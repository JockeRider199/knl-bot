...
import { type SlashCommand } from "../../models/slash-command";
	...
	async exec(interaction: ChatInputCommandInteraction, client: Client) {
		if (interaction.options.getSubcommand() === "setup") {
			...
			if (
				...
				await Database.removeTicket(interaction.channelId);
		...
		switch (buttonId) {
			...
			case "close": {
				if (
					...
					await Database.closeTicket(interaction.channelId);
					const channel = interaction.channel as TextChannel;
			...
			case "open": {
				await interaction.message.delete();
				const ownerId = await Database.openTicket(interaction.channelId);
